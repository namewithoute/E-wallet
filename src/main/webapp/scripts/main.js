...
function getName() {
   document.getElementById('autoFillName').innerHTML=''
    ...
    console.log(sdt)
    fetch('http://localhost:5000/get-name-user?phone='+sdt, {
        method: 'GET',
        credentials: 'include',
        headers: {
            'Content-Type': 'application/json',
...
//        body: JSON.stringify({ sdt: sdt })
        ...
        .then(function (name) {
            if (name.fullName) {
                ...
                document.getElementById('autoFillName').innerHTML = `
            <p class="text mb-1 text-danger">Tên người nhận: ${name.fullName}</p>
            `
            }
            if (name.err) {
                document.getElementById('autoFillName').innerHTML = `
...
socket.on('message',function(data){
    ...
    <div>
        <div class="message-data text-right">
            <span class="message-data-time">${data.createAt}</span>
        </div>
    </div>

    <div class="message other-message float-right" id="sender">${data.message} </div>
</li>`
...
function sendMess() {
    ...
    var idBox = localStorage.getItem('boxID')

    socket.emit('message', { message, user, idBox })
}
